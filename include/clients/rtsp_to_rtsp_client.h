...
/**
 * RTSPToRtspClient — RTSP man-in-the-middle proxy.
 *
...
{
public:
    explicit RTSPToRtspClient(EpollLoop *loop, BufferPool &pool,
                            const sockaddr_in &client_addr, int client_fd,
                            const RtspMitmConfig &config,
                            const std::string &first_request);

    static RtspMitmConfig resolve_upstream(const std::string &first_request);
    ~RTSPToRtspClient() override;
...
private:
    ...
    /* ------------------------------------------------------------------ */
    /* RAII fd wrapper (identical pattern to RTSPToHttpClient)                   */
    /* ------------------------------------------------------------------ */
    class FdGuard
    {
    public:
